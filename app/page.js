    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        try {
          ...
          setCopiadoImg(true);
          setTimeout(() => setCopiadoImg(false), 2500);
        } catch (e) {
          setError('No se pudo copiar la imagen al portapapeles: ' + e.message);
        }
        setGenerandoImg(false);
      });
    } catch (e) {
      setError('No se pudo generar la imagen: ' + e.message);
      setGenerandoImg(false);
    }
  }
          ...
          <div style={{ marginBottom: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={copiarCorreo} style={{ padding: '6px 14px', background: 'white', color: 'var(--imss-verde-oscuro)', border: '1px solid var(--imss-verde)', borderRadius: 4, fontSize: 12 }}>
                  {copiado ? '¡Copiado!' : 'Copiar con formato'}
                </button>
                <button onClick={copiarComoImagen} disabled={generandoImg} style={{ padding: '6px 14px', background: 'var(--imss-verde)', color: 'white', border: 'none', borderRadius: 4, fontSize: 12, opacity: generandoImg ? 0.6 : 1 }}>
                  {generandoImg ? 'Generando...' : copiadoImg ? '¡Imagen copiada!' : 'Copiar como imagen'}
                </button>
              </div>
            </div>
            <div
